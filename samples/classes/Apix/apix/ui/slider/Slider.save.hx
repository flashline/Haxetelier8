...
class Slider extends UICompo    {  
	...
	*/
	...
	function startDrag (e) {	
		...
		var o = selectors.objectOf("elem", el);
...
class SliderLoader extends UICompoLoader   { 
	...
	static public function __init (?skinName = "default", ?pathStr:String)  {
		...
		UICompoLoader._push( SliderLoader.__load,UICompoLoader.baseUrl+pathStr,skinName) ;
	}
	/**
	 * private static
	 */
	static function __load (fromPath:String,sk:String)  {
		...
		h.onData = __onData;	
		h.request(false);
		SliderLoader.__tmpSkinName = sk;
		SliderLoader.__tmpFromPath = fromPath;	
	}
	
	static function __onData (result:String)  {
		var tmpCtnr = Common.getElem(UICompoLoader.TMP_CTNR_ID);
		result = __strReplace(result, UICompoLoader.TMP_IMG_URL_PREFIX, SliderLoader.__tmpFromPath ) ;
		...
		var styleElArr = Common.getElemsByTagName("style");
		if (styleElArr.length == 0) {
			tmpStyleEl.textContent = styleContent ;
			Common.head.appendChild(tmpStyleEl);
		} else {
			var styleEl = styleElArr[0];
			styleEl.textContent+=styleContent;		
		}
		var el:Elem = (untyped tmpCtnr.getElementsByClassName("apix_loader_ctnr")[0]) ;
		var skinContent=el.inner();
		SliderLoader.__compoSkinList.push({skinName:SliderLoader.__tmpSkinName,skinContent:skinContent,skinPath:SliderLoader.__tmpFromPath}); 		
		tmpCtnr.inner("");		
